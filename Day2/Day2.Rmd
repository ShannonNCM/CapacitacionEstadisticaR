...
Se va a bucar obtener un modelo para analizar el sueldo y como la edad
determina el sueldo, la edad es la variable independiente y el salario
la variable dependiente

Aqui se quiere un modelo para el salario en funcion del nivel de
escolaridad nivel educativo B1P00B07A

```{r}
datatest <- basedatos %>% select(B1P00B07A, B1P02B08) %>%
...
si nos preguntamos cuanto mas gana el nivel 5 respecto a todos los demas
niveles, serian 3243
...
Se crea un nivel por cada variable El nivel 1 que era de menor
importancia esta oculto en el intercepto En el ejemplo de este coso
multiple la comparacion se hace respecto al nivel 1

------------------------------------------------------------------------

Cuando las variables dependientes no son continuas se tienen modelos no
lineales, binarias o ficticias, 01 se puede usar un modelo de regresion
pero carece de interpretacion

Se va a ver el salario con nivel educacional en estos casos se toma la
mediana ya que la media es muy sensible a los valores extremos \*\*\* se
usan salarios altos y bajos alto \> a la mediana bajo \< a la mediana
...
con logit y probit analizar genero por salario, edad (separando menores
y mayores de edad), tamano de empresa (efectos agrupados y luego
desagrupados, ie una unica variable que tenga los 5, y todas las
variables posibles)

# Sexo (mujeres) versus Edad, Salario, Tamaño de empresa (quiere decir que la variable que mayor interes aca son las mujeres)

variable dpendiente: sexo sexo: B1P00A02 (mujer y hombre) edad: B1P00A03
(media y dividimos) salario: B1P02B08 (solo salario) tamano empresa:
tiene que ver con el numero de empleados (con los tamanos de antes)
...
meanage <- mean(basedatos$B1P00A03)
fage <- function(x){
...
```{r}
reg_logit = glm(genero~ edad+salario+tamano, data=base, family=binomial(link='logit'))
summary(reg_logit)
